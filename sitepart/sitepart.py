...
# Обновляем запись
@sitepart.route('/update/<int:id>/', methods=['PUT'])
@swag_from('update.yml')
def update(id: int):
    ...
    rec[0]['country'] = args.get('country')

    rec[0]['language'] = args.get('language')

    rec[0]['speakers'] = int(args.get('speakers'))

    rec[0]['letters'] = int(args.get('letters'))

    rec[0]['words'] = int(args.get('words'))

    return jsonify(rec), HTTPStatus.OK

# Обновляем поля записи
@sitepart.route('/change/<int:id>/', methods=['PATCH'])
